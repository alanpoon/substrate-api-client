...
#[cfg(feature = "std")]
impl<P: Pair> Api<P> {
    pub async fn balance_transfer(&self, to: GenericAddress, amount: u128) -> BalanceTransferXt<P> {
            compose_extrinsic!(
            self,
            BALANCES_MODULE,
            BALANCES_TRANSFER,
    ...
    pub async fn balance_set_balance(&self, who: GenericAddress, free_balance: u128, reserved_balance: u128) -> BalanceSetBalanceXt<P> {
            compose_extrinsic!(
            self,
            BALANCES_MODULE,
