        ...
        MultiSignature: From<P::Signature>,
{
    ...
    pub async fn set_signer(mut self, signer: P) -> Self {
    ...
    async fn _get_nonce(url: String, signer: [u8; 32]) -> u32 {
    ...
    async fn _get_storage(
        ...
        Self::_get_request(url, jsonreq.to_string()).await
    }

    async fn _get_storage_double_map(
        url: String,
        module: &str,
        storage_key_name: &str,
        first: Vec<u8>,
    ...
    pub async fn get_nonce(&self) -> Result<u32, &str> {
        match &self.signer {
            Some(key) => {
                ...
                Ok(Self::_get_nonce(self.url.clone(), arr).await)
            },
            None => Err("Can't get nonce when no signer is set"),
        }
    }

    pub async fn get_free_balance(&self, address: &AccountId32) -> U256 {
        let id: &[u8; 32] = address.as_ref();
        let result_str = self
            .get_storage("Balances", "FreeBalance", Some(id.to_owned().encode())).await
            .unwrap();
        hexstr_to_u256(result_str).unwrap()
    }

    pub async fn get_request(&self, jsonreq: String) -> Result<String, &'static str> {
        Self::_get_request(self.url.clone(), jsonreq).await
    }
    ...
    pub async fn get_storage(
        ...
        Self::_get_storage(self.url.clone(), storage_prefix, storage_key_name, param).await
    }
    ...
    pub async fn send_extrinsic(&self, xthex_prefixed: String) -> Result<Hash,&'static str> {
        ...
        let result = result_in.signal_cloned().to_future().await;
        Ok(hexstr_to_hash(result).unwrap())
    }

    pub async fn subscribe_events(&self, sender: Mutable<String>) {
        debug!("subscribing to events");
        let key = storage_key_hash("System", "Events", None);
        let jsonreq = json_req::state_subscribe_storage(&key).to_string();

        rpc::start_event_subscriber(self.url.clone(), jsonreq.clone(), sender).await
    }
}
