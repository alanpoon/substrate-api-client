        ...
        P: Pair,
{
    ...
    pub async fn get_free_balance(&self, address: &AccountId) -> U256 {
        let result_str = self
            .get_storage("Balances", "FreeBalance", Some(address.0.encode())).await
            .unwrap();
    ...
    pub fn subscribe_events(&self, sender: Mutable<String>) {
        debug!("subscribing to events");
        let key = storage_key_hash("System", "Events", None);
        let jsonreq = json_req::state_subscribe_storage(&key).to_string();

        rpc::start_event_subscriber(self.url.clone(), jsonreq.clone(), sender);
    }
}